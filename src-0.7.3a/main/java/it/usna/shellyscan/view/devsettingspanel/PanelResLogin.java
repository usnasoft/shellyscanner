...
import it.usna.shellyscan.view.DialogDeviceSettings;
import it.usna.shellyscan.view.DialogDeviceSettings.Gen;
...
//		textFieldPwd.setColumns(10);
	...
	public String apply() {
		...
		final String pwd = new String(textFieldPwd.getPassword());
		...
		for(int i=0; i < devices.size(); i++) {
			String msg;
			if(enabled) {
				msg = loginModule.get(i).set(user, pwd, credsProvider);
			} else {
				msg = loginModule.get(i).disable();
			}
			if(msg != null) {
