...
public class PanelMQTTG1 extends AbstractSettingsPanel {
	...
	public PanelMQTTG1(List<ShellyAbstractDevice> devices) {
		...
		rdbtnCleanSessionYes = new JRadioButton(LABELS.getString("true_yn"));
		...
		rdbtnCleanSessionNo = new JRadioButton(LABELS.getString("false_yn"));
		...
		rdbtnRetainYes = new JRadioButton(LABELS.getString("true_yn"));
		...
		rdbtnRetainNo = new JRadioButton(LABELS.getString("false_yn"));
	...
	public String apply() {
		...
		if(enabled) {
			...
			// Validation
			if(server.isEmpty() || ((user.isEmpty() || pwd.isEmpty()) && chckbxNoPWD.isSelected() == false)) {
