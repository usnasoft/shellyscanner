			...
			try {
				...
				if(fc.showOpenDialog(w) == JFileChooser.APPROVE_OPTION) {
					...
					if(test.containsKey(Restore.ERR_RESTORE_HOST) &&
							JOptionPane.showConfirmDialog(w, String.format(LABELS.getString("msgRestoreDifferent"), test.get(Restore.ERR_RESTORE_HOST)),
						...
						Msg.errorMsg(w, LABELS.getString(test.get(Restore.ERR_RESTORE_MSG)));
						return;
					} else {
						if(test.containsKey(Restore.WARN_RESTORE_MSG)) {
							Msg.warningMsg(w, LABELS.getString(test.get(Restore.WARN_RESTORE_MSG)));
						}
					...
					if(ret == null || ret.length() == 0) {
						if(device.rebootRequired())	{
							...
							if(JOptionPane.showOptionDialog(w, LABELS.getString("msgRestoreSuccessReboot"), device.getHostname(),
