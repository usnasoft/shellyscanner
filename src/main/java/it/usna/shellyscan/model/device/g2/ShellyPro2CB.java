...
/*
 * NOT USED - based on an obsolete hardware
 */
public class ShellyPro2CB extends AbstractProDevice implements ModulesHolder, InternalTmpHolder {
	...
	protected void fillSettings(JsonNode configuration) throws IOException {
		...
		voltmeterName = configuration.get("voltmeter:0").get("name").asString("");
	}
	...
	protected void restore(Map<String, JsonNode> backupJsons, List<String> errors) throws InterruptedException {
		...
		errors.add(postCommand("CB.SetConfig", RestoreUtil.createIndexedRestoreNode(config, "voltmeter", 0)));
	}
}